...
export function Payment() {
  ...
  const regularAmount: number | undefined = location.state?.regularAmount;
  const label: string = location.state?.label ?? 'Total Due';
  const recurring: boolean = !!location.state?.recurring;
  const renewalNote: string | undefined = location.state?.renewalNote;
            ...
            {amountDisplay && (
              ...
              >
                ...
                <div className="flex items-baseline gap-2">
                  {regularAmount != null && (
                    <span className="text-2xl font-semibold text-white/50 line-through decoration-2 decoration-red-400/80">${regularAmount.toFixed(2)}</span>
                  )}
                  ...
                  {recurring && <span className="text-lg font-semibold text-amber-300/90 ml-1">/mo</span>}
                </div>
                {renewalNote && (
                  <p className="text-[11px] uppercase tracking-widest text-amber-200/80 font-semibold mt-2">{renewalNote}</p>
                )}
              </motion.div>
            )}
